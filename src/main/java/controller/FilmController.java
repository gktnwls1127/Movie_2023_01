...
public class FilmController {
    ...
    public FilmDTO selectOne(int id){
        ...
        if (list.contains(new FilmDTO(id))){
            return new FilmDTO(list.get(list.indexOf(new FilmDTO(id))));
    ...
    public String selectFilmTitle(int id) {
        FilmDTO f = new FilmDTO(id);
        int index = list.indexOf(f);
        return list.get(index).getTitle();
    }

    public boolean validateFilmId(int filmId){
        for (FilmDTO f : list){
            if (filmId == f.getId()){
                return false;
            }
