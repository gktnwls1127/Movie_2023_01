...
public class MemberViewer {
    ...
    private void register(){
        ...
        if (!username.equalsIgnoreCase("X")) {
            ...
            String message2 = "사용하실 비밀 번호를 입력해주세요.";
    ...
    private void approveRequest(){
        String message = "1. 평론가 신청 목록 2. 관리자 신청 목록 3. 뒤로가기";
        int userChoice = ScannerUtil.nextInt(SCANNER, message);
        if (userChoice == 1) {
    ...
    private void printRequest(ArrayList<MemberDTO> list, String level){
        ...
        while (userChoice != 0 && !list.contains(new MemberDTO(userChoice))){
            System.out.println("해당 번호는 유효하지 않은 회원 번호 입니다.");
        ...
        if (userChoice != 0){
            if (level.equalsIgnoreCase("평론가")){
                ...
                reviewViewer.updateByWriterRank(userChoice);
    ...
    private void delete(){
        if (logIn.getId() == 1){
            ...
            String message = "정말 탈퇴하시겠습니까? Y/N";
